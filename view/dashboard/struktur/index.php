...
$totalAnggota = count(getStrukturAll());
...
<a class="bg-primary text-on-primary font-caption text-caption px-6 py-3 rounded-full flex items-center gap-2 hover:shadow-lime-glow transition-all duration-300 group whitespace-nowrap" href="<?= APP_BASE ?>/dashboard/struktur/form">
<span class="material-symbols-outlined text-[20px] transition-transform group-hover:rotate-90">add</span>
Tambah Jabatan
</a>
</div>

<div class="bg-glass-fill backdrop-blur-md rounded-[20px] border border-glass-border p-4 md:p-stack-lg relative overflow-hidden group hover:border-primary/40 transition-colors duration-500 hover:-translate-y-[3px]">
...
<form id="struktur-filter" class="flex flex-col md:flex-row items-stretch md:items-center gap-3 mb-4 relative z-10" onsubmit="return false;">
<input class="flex-1 min-w-0 bg-surface-container-highest border border-glass-border rounded-xl py-3 pl-4 pr-4 text-body-md font-body-md text-on-surface focus:outline-none focus:border-primary focus:shadow-lime-glow transition-all placeholder:text-on-surface-variant/50" name="q" data-live-search placeholder="Cari nama atau jabatan..." type="text"/>
...
<th class="py-4 px-4 font-medium">Kontak</th>
...
document.addEventListener('DOMContentLoaded', function () {
    AdminUI.initAjaxTable({
        ...
        actions: {
            delete: function (btn) {
                AdminUI.confirmModal('Hapus Jabatan', btn.dataset.pesan || 'Yakin ingin menghapus data ini?', 'Hapus', function () {
